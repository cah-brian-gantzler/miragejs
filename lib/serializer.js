...
class Serializer {
  ...
  */
  ...
  _attrsForModel(model) {
    ...
    if (this._transforms) {
      ...
      attrs = Object.keys(attrs).reduce((memo, attr) => {
        let transform = this._transforms[attr];
  ...
  _resolveTransforms(model) {
    let whitelist = this.includeAttrs;
    let _transforms = undefined;

    // backward compat
    if (Array.isArray(this.attrs)) {
      if (!whitelist) { // should we merge the two?
        whitelist = this.attrs;
      }
      _transforms = {};
    } else {
      _transforms = this.attrs || {};
    }
    ...
    Object.keys(model.attrs).forEach(item => {
      ...
      if (_transforms[item]) {
        transform = typeof _transforms[item] === 'string'
            ? {key: _transforms[item]}
            : _transforms[item];
      } else {
        transform = {key: item};
      }

      this._transformDefaults(transform, item, whitelist, fks);

      this._transforms[item] = transform;
  ...
  _transformDefaults(transform, property, whitelist, fks) {
