...
class Serializer {
  constructor(registry, type, request = {}) {
    ...
    */
    ...
    this.transforms = this.transforms || undefined; // this is just here so I can add the doc comment. Better way?
  }
  ...
  */
  ...
  _attrsForModel(model) {
    ...
    // Do the transform after the fks have been removed
    if (this.transforms) {
      // this is more exclusion, look for ones to remove
      attrs = Object.keys(attrs).reduce((memo, attr) => {
        let transform = this.transforms[attr];
        if (transform.serialize === false) {
          delete memo[attr];
        }
        return memo;
      }, attrs);
  ...
  /**
    The attempt here is to replace serializeIds, embed, this.attrs as an array and sideload with a new attrs structure

    this.attrs =
    {
      miragePropertyName: {
        key: "externalName",
        serialize: value,        // For attributes
                                 //     true = serialize the attribute
                                 //     false = do not serialize the attribute
                                 // For foreign keys
                                 //     false = do not serialize the foreign key
                                 //     ids = serialize as id
                                 //     records = embed the record
        sideload: true
      }
    }

    If a value is not specified, the following rules will apply

    @param model
   */
  _resolveTransforms(model) {
    this.transforms = this.transforms || {};

    let fks = model.fks;

    Object.keys(model.attrs).forEach(item => {
      let transform;

      if (this.transforms[item]) {
        transform = typeof this.transforms[item] === 'string'
            ? {key: this.transforms[item]}
            : this.transforms[item];
      } else {
        transform = {key: item};
      }

      this.transforms[item] = transform;

      this._setTransformDefaults(transform, item, this.attrs, fks);
    });
  }

  // We are trying to set reasonable defaults based on other settings
  // really this format should replace serializeIds, include, embed, and the array version of attrs
  _setTransformDefaults(transform, property, whitelist, fks) {
    // console.log(property, transform);
    if (transform.serialize === undefined) {
      let value = 'ids';
      // if there is a white list and its not in it, dont include
      if (whitelist && !whitelist.find(item => item === property)) {
        value = false;
      } else {
        // is this a attr?
        let isFk = fks.find(fk => fk === property);
        if (!isFk) {
          value = true;
        } else {
          // Find a good default
          // serialize defaults to ids
          // serializeIds:never      false
          //             :include   if in include and embed: true     records
          if (this.serializeIds === 'never') {
            value = false;
          } else if (this.serializeIds === 'included') {
            // we have to assume that include none means include all in this case
            // its only if this.attrs is not an array
            if (this.include.length === 0) {
              value = false;
              let include = this.getKeysForIncluded().find(item => item === property);
              if (include && this.embed) {
                value = 'records';
              }
            }
          }
      ...
      transform.serialize = value;
    }

    if (transform.sideload === undefined) {
      transform.sideload = this.sideload;
    }
